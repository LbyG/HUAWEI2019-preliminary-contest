...
void overall_schedule::car_path_regulation() {
    ...
    for (map<int, car>::iterator iter = this->cars.begin(); iter != this->cars.end(); iter ++) {
        ...
        car_vec.push_back(new_car);
        n ++;
    }
    typedef pair<int, int> VecItem;
    vector<VecItem> pair_v;
    for (vector<car>::iterator car_iter = car_vec.begin(); car_iter != car_vec.end(); car_iter ++) {
            ...
            cross_arrive_time[car_iter->get_from()] = 0;
        ...
        pair_v.push_back(pair<int, int>(cross_arrive_time[car_iter->get_to()], car_iter->get_id()));
    }
    sort(pair_v.begin(), pair_v.end());
    this->cars_to_regulate_path.clear();
    for (int i = 10001; i < n; i ++) {
        //cout << riter->first << " " << riter->second << endl;
        car new_car(this->cars[pair_v[i].second]);
        new_car.set_schedule_start_time(new_car.get_plan_time());
