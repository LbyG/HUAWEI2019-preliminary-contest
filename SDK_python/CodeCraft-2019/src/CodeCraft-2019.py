...
# cross_with_road[road[from]].append(road)
# if isDuplex == 1 cross_with_road[road[to]].append(road)
    roads = {}
    road_attr = []
    cross_with_road = {}
    for cross_id in crosses: cross_with_road[cross_id] = list()
    with open(road_path, 'r') as r_file:
        ...
        for info in road_info:
            ...
            roads[road["id"]] = road
            cross_with_road[road["from"]].append(road.copy())
            if road["isDuplex"] == 1:
                road_reverse = road.copy()
                road_reverse["from"] = road["to"]
                road_reverse["to"] = road["from"]
                cross_with_road[road_reverse["from"]].append(road_reverse)
    return cross_with_road, roads, road_attr
...
def traffic_regulation(cars, crosses, roads, cross_with_road):
    ans = []
    start_time = 0
    count = 0
    heap = []
    for car in cars:
        ...
        run_time, path = find_shortest_path(car, cross_with_road)
...
# to read input file
    ...
    cross_with_road, roads, road_attr = read_road_file(road_path, crosses)
# process
    ans = traffic_regulation(cars, crosses, roads, cross_with_road)
